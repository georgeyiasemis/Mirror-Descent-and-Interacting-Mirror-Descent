...
print('Variance of NLA: {}'.format(np.array(iters_NLA['scatter_error'])[200:].var()))
print('Variance of INLA: {}'.format(np.array(iters_INLA['scatter_error'])[200:].var()))
plt.figure()
plt.hist(np.array(iters_NLA['scatter_error'])[200:], label=r'NLA, $\epsilon$ = 0.7,  $N_{iid}$ = 10')
plt.hist(np.array(iters_INLA['scatter_error'])[200:], label='INLA, $\epsilon$ = 0.7,  $N_p$ = 10')
plt.title(r'$log_{10}\frac{||\, \hat{\Sigma}\,-\,\Sigma\,||_F^2}{||\,\Sigma\,||_F^2}$ after 200 iterations', fontsize=20)
